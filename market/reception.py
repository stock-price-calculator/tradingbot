...
class Kiwoom_Receive_Market_price:
    ...
    def receive_minutes_chart_data(self,sTrCode, sRQName, sRecordName):
        ...
        for i in range(repeat):
            current_price = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "현재가")
            volume = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "거래량")
            open_price = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "시가")
            high_price = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "고가")
            low_price = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "저가")
            standard_minute = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "체결시간")
    ...
    def receive_day_chart_data(self,sTrCode, sRQName, sRecordName):
        ...
        for i in range(repeat):
            ...
            high_price = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "고가")
            low_price = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "저가")
            standard_day = self.Kiwoom.get_comm_data(sTrCode, sRecordName, i, "일자")
